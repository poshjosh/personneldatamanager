    ...
    public <T> T search(Class valueType, String column, Object value, T outputIfNone) {
        ...
        if(Localgovernmentarea.class.isAssignableFrom(valueType) && value instanceof Map) {
            ...
            if((refVal = map.get(refCol)) != null) {
                ...
                final Stateoforigin soo = super.search(Stateoforigin.class, refCol, refVal, null);
                
                if(soo != null) {
                   
                    final List<Localgovernmentarea> lgaList = soo.getLocalgovernmentareaList();

                    String prefix = this.getApp().getConfig().getProperty(ConfigNames.DEFAULT_LOCALGOVERNMENTAREA_PREFIX);
